...
The `<text>` element is different from the `<view>` in layout. It does not support setting typesetting and related properties such as `display`. The `<text>` element has its own layout method. The `<text>` element provides an [inline formatting context](https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_inline_layout/Inline_formatting_context). In Lynx, except for the elements in the `<text>` element, all other elements are block-level elements. When an element is inside the `<text>` element, the `<text>` element provides an inline formatting context, so that the elements in it have the layout characteristics of an inline element. The elements do not occupy a line alone, but are arranged in the same line as the surrounding text content.

### Nested Text

Nested text refers to text subelements within a text element, similar to `Inline Elements` in the Web. They can be written as `<text>` and are generally used for setting additional text styles or adding click events.

Unlike text elements, nested text only supports some `color/font/background-related` properties and does not support view properties such as `opacity/margin/padding` or paragraph properties like `line-height/text-align`.

:::note

`line-height/text-align` only supports being written on the text element and not on nested text.
...
Nested images are text subelements and can be written as `<image>`, similar to `Inline Elements` in the Web, generally used for mixed text and image layout.
...
[text](/api/elements/built-in/text.html#text)：built-in element in Lynx and used to display text content.

<LegacyCompatTable metadata="elements/text" />

### nested text

[nested text](/api/elements/built-in/text.html#nested-text)： within a `<text>` element and used to set additional text styles or adding click events.

<LegacyCompatTable metadata="elements/nested-text" />

### nested image

[nested image](/api/elements/built-in/text.html#nested-image)： within a `<text>` element and used to mix text and image layouts.
