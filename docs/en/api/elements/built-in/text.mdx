...
Nested text refers to text subelements within a text element. They can be written as `<text>` and are generally used for setting additional text styles or adding click events.
...
Nested images are text subelements and can be written as `<image>`, generally used for mixed text and image layout.
...
| [text-align](api/css/properties/text-align.mdx)           | Yes  | No          | No           | No          | Sets text alignment within the text element                                                                         |
