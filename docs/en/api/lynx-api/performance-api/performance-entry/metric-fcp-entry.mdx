...
FCP is a key performance metric that measures the time taken for the [first-frame rendering](/guide/spec#first-frame-rendering) of lynx app to complete. It refers to the time taken for users to see any content (such as text or images) on a lynx app for the first time. `MetricFcpEntry` interface provides timing information about this metric, inheriting from [`PerformanceEntry`](../performance-entry).
