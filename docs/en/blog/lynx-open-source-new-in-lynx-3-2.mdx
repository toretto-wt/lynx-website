...
In previous versions, the implementation used in Lynx for Web was a single-threaded version. In the [0.12.0](https://www.npmjs.com/package/@lynx-js/web-core/v/0.12.0) version, Rspack Worker capabilities will be used to create Workers, thereby achieving multi-threaded rendering and truly delivering high performance similar to Lynx Android and iOS.

### Multi-Thread Render

In the multi-thread rendering design of Lynx for Web, the main thread is only responsible for rendering and the logic related to the Document Object Model (DOM). The JavaScript logic of the pages, such as data processing and the difference calculation of the Virtual Document Object Model (VDOM), will be executed concurrently in worker threads. The more complex the JavaScript logic is, the more obvious the performance advantages of the multi-thread architecture will be.
...
Actual data shows that in some business scenarios, the multi-threaded rendering of Lynx for Web achieves approximately a 30% performance improvement compared to the main thread version. Meanwhile, a [test case](https://github.com/PupilTong/performance-explanation-react-vs-reactlynx) is also provided. If you are interested, you can explore based on that.
