...
In the multi-threaded rendering design of Lynx for Web, the main thread is only responsible for rendering and the logic related to the Document Object Model (DOM). The JavaScript logic of the pages, such as data processing and the difference calculation of the Virtual Document Object Model (VDOM), will be executed concurrently in worker threads. The more complex the JavaScript logic is, the more obvious the performance advantages of the multi-threaded architecture will be.
