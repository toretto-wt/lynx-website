...
- **Rendering pipeline events (`pipeline`)**
  ...
  - [`LoadBundleEntry`](/api/lynx-api/performance-api/performance-entry/load-bundle-entry): Performance event for [TemplateBundle](/api/lynx-native-api/template-bundle) loading and first-frame rendering
...
For example, if you want to measure the latency from the end of first-frame rendering to the first important data update, you can combine `LoadBundleEntry` and `PipelineEntry` to compute a custom metric `waitingDuration` as shown below. It helps monitor the speed of network requests, file IO, etc., and pinpoint causes of performance regressions.
...
### Q1: If my page uses [_Instant First-Frame Rendering (IFR)_](/guide/interaction/ifr.html), do I still need to mark a Timing Flag? {#q1-if-my-page-uses-instant-first-frame-rendering-ifr-do-i-still-need-to-mark-a-timing-flag}
...
### Q2: My page uses [_Instant First-Frame Rendering (IFR)_](/guide/interaction/ifr.html). How can I get the ActualFMP performance metric? {#q2-my-page-uses-instant-first-frame-rendering-ifr-how-can-i-get-the-actualfmp-performance-metric}
...
1. First-frame rendering produced no pixels, e.g., LynxView width/height is 0, LynxView is off-screen, or LynxView wasn’t added to the window.
