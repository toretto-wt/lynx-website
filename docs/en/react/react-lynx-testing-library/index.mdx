...
:::info No need to install @lynx-js/environment
`@lynx-js/environment` is already included in `@lynx-js/react-lynx-testing-library`'s `dependencies`. `createVitestConfig` will configure `@lynx-js/environment` to make it available, no additional configuration is needed.
:::
...
```jsx
import { fireEvent, render } from '@lynx-js/react-lynx-testing-library';
import { expect } from 'vitest';
...
For more usage, please refer to the [test cases](https://github.com/lynx-family/lynx-stack/tree/main/packages/testing-library/react-lynx-testing-library/src/__tests__) maintained in the ReactLynx Testing Library source code.
