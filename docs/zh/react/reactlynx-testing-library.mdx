...
describe('IFR Testing', () => {
  it('will render wrong result if has top level side effects', () => {
...
describe('IFR Testing', () => {
  it('will render correct result if does not have top level side effects', () => {
    const Comp = () => {
      return <text>{__BACKGROUND__ ? 'background' : 'main thread'}</text>;
    };
