...
- **渲染流水线事件 (`pipeline`)**
  ...
  - [`LoadBundleEntry`](/api/lynx-api/performance-api/performance-entry/load-bundle-entry)：[TemplateBundle](/api/lynx-native-api/template-bundle) 加载和首帧渲染性能事件
...
假如你希望关注从首帧渲染结束到首次重要数据更新的延迟，你可以像下面的代码这样灵活组合 `LoadBundleEntry` 和 `PipelineEntry` 计算出一个属于你的性能指标 `waitingDuration`，它可以帮助你监测网络请求、文件读取等行为的速度，精准定位页面性能变坏的原因。
...
### Q1: 我的页面是[_首帧直出 (Instant First-Frame Rendering，IFR)_](/guide/interaction/ifr.html)的情况下，还需要标记 Timing Flag 吗？ {#q1-我的页面是首帧直出-instant-first-frame-renderingifr的情况下还需要标记-timing-flag-吗}
...
<h3 id="q2-我的页面是首帧直出-instant-first-frame-renderingifr的那我需要如何拿到-actualfmp-性能指标呢">
  Q2: 我的页面是首帧直出 (Instant First-Frame Rendering，IFR)
  的，那我需要如何拿到 ActualFMP 性能指标呢？
</h3>
...
1. **首帧渲染为空**，比如 LynxView 页面宽高为 0、LynxView 不在屏幕内、LynxView 没有被添加到窗口内等原因都会导致渲染为空。
