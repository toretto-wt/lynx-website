...
Lynx 3.2 对框架、引擎和工具进行了全面更新，包括 [ReactLynx 测试工具](#reactlynx-testing-library)、[`llms.txt` 支持](#网站llmstxt-支持)、[Grid 布局新特性](##css-网格布局minmaxmax-contentfit-content)、[`<list>`](#list-元件优化) 和 [`<text>`](#text-可被定制为跨节点选择) 等等，旨在为开发者带来更熟悉的开发体验和更多的能力。该版本拥有来自 61 位贡献者（包括一些首次贡献者）的超过 [296 次提交](https://github.com/lynx-family/lynx/releases/)。让我们一起来看看有哪些新特性！
...
通过升级到 [Rspress v2](https://v2.rspress.dev/) 并利用新的 [LLM 插件](https://x.com/rspack_dev/status/1917844832149725695)，Lynx 网站现已完全支持 [https://lynxjs.org/llms.txt](https://lynxjs.org/llms.txt) 和 [https://lynxjs.org/llm-full.txt](https://lynxjs.org/llm-full.txt)，以帮助 AI 更好地理解 Lynx，提升你使用 AI 辅助编程的体验。对于每个页面，你都可以通过将 `.html` 扩展名替换为 `.md` 来获取原始的 markdown 文件。
