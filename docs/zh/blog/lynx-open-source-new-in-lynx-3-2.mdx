...
Lynx 3.2 对引擎、框架和工具进行了全面更新。其中，`List` 元件、Grid 布局、跨文本选择、Rspeedy 工具、ReactLynx 测试工具、Lynx for Web 以及 DevTool 均得到了改进，使得开发过程更加高效，性能也有了显著提升。值得一提的是，Lynx 3.2 拥有来自 61 位贡献者的超过 [296 次提交](https://github.com/lynx-family/lynx/releases/)；其中包含许多社区开发者，在文档、工具、功能上对 Lynx 开源给予很大的帮助。现在，让我们一起开始探索吧！

## 引擎

### `<list>` 元件

Lynx 3.2 中对 `<list>` 元件的功能做了完善以提供更好的开发体验，同时还对其首屏、滚动做了深度的优化，使得 `<list>` 具备更加优秀的性能表现。

- **功能更新**

1. 完善[**磁吸定位能力**](https://lynx.bytedance.net/api/components/built-in/list.html#item-snap)，使分页滚动体验更加顺畅。基于这一改进，开发者可以更便捷的构建沉浸式的信息流体验，跑马灯这类需要精确控制位移的复杂交互也能轻松实现
...
2. 增加对 `<list-item>` 上 [**z-index**](https://lynxjs.org/zh/api/css/properties/z-index.html) 的支持，能够更方便的调整列表项的视图层级
...
1. 多线程配合子节点的**批量渲染**，优化渲染节点的整体耗时，从而提升 `<list>` 首屏或者数据更新时的渲染性能。

2. `<list>` **异步渲染模式**的优化，增加多种策略来解决异步调度产生渲染完整性问题，带来更好的滚动体验。
...
### 基于 `lynx.queueMicrotask` 实现的 `Promise`

Lynx 3.2 中实现了微任务调用 API `lynx.queueMicrotask`，从而实现任务插队需求；基于此，支持了更为标准的 `Promise`。相应的 `Promise` 的执行差异，可以通过以下代码来验证，非基于 `lynx.queueMicrotask` 实现的 `Promise` （ 3.2 之前的版本）与主流浏览器不一致。
...
在最新的 ReactLynx 中实现了 [ReactLynx Testing Library](/react/react-lynx-testing-library/index)，它是基于 Lynx 测试环境（[`@lynx-js/test-environment`](/react/react-lynx-testing-library/lynx-test-environment.html)），为 ReactLynx 实现的一套测试解决方案。整体基于 `jsdom` 来实现 [Element PAPI](https://lynxjs.org/api/engine/element-api)，可以直接使用诸如 [`@testing-library/dom`](https://github.com/testing-library/dom-testing-library) 和 [`@testing-library/jest-dom`](https://github.com/testing-library/jest-dom) 等工具来辅助测试。

- **适用场景**

ReactLynx Testing Library 适用于对 ReactLynx 组件进行单元测试。基于 `@testing-library/dom`（用于页面元素的查询和事件触发），将 ReactLynx 的渲染过程抽象成与标准测试库类似的 API，如 `render`等。用户还可以对页面中的元素进行断言，例如使用 `@testing-library/jest-dom` 中的 `toBeInTheDocument` 来判断元素是否存在于页面中。

- **使用指南**

1. ReactLynx Testing Library 已集成到 [`@lynx-js/react`](https://www.npmjs.com/package/@lynx-js/react) 包的 `testing-library` 子目录中，可直接使用。之后配置 [Vitest](https://vitest.dev/) 测试框架，您就可以开始编写测试并运行它们了。
...
2. 以下是一个完整的示例，用于来做基础渲染测试的 case，更多的测试使用方式可以参考 [ReactLynx Testing Library](https://github.com/lynx-family/lynx-stack/tree/main/packages/react/testing-library/src/__tests__)。
...
[Lynx for Web](https://github.com/lynx-family/lynx-stack/tree/main/packages/web-platform) 是基于浏览器 API 来模拟实现 Lynx，最终将 Lynx 代码运行到浏览器一种方式。通过这种方式，Lynx for Web 能够帮助 Lynx 扩展到更多平台，如桌面设备、电视或物联网（IoT）设备。在 Lynx for Web [0.12.0](https://www.npmjs.com/package/@lynx-js/web-core/v/0.12.0) 版本中，借助 Rspack Worker 能力创建 Worker，完整实现了 Lynx for Web 多线程渲染，达到与 Lynx Android 和 iOS 相近的高性能表现。
