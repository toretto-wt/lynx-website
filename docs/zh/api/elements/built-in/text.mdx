...
`<text>` 元件在布局上和 `<view>` 不同，不支持设置 `display` 等排版及相关属性，`<text>` 内部有自身的布局方式。在 Lynx 里，除了在 `<text>` 元件内的元件，其他元件都是块级元件。当元件处于 `<text>` 元件内部时，`<text>` 元件如同提供了一个行内格式化上下文（[Inline formatting context](https://developer.mozilla.org/zh-CN/docs/Web/CSS/CSS_inline_layout/Inline_formatting_context)），使得其中的元件具有类似行内元件的布局特性，元件不会独占一行，而是与周围文本内容在同一行内排列。

### 嵌套 text

嵌套 text 是 `<text>` 内的子元件，类似 Web 中 `Inline Elements`， 可以写成 `<text>`，一般用于设置额外的文字样式或者增加点击事件。
...
嵌套 image 是 `<text>` 内的子元件，类似 Web 中 `Inline Elements`，可以写成 `<image>`，一般用于图文混排。
...
[text](/api/elements/built-in/text.html#text)：Lynx 内置元件，用来显示文本内容。

<LegacyCompatTable metadata="elements/text" />

### 嵌套 text

[嵌套 text](/api/elements/built-in/text.html#嵌套-text)： `<text>` 内的子元件， 用于设置额外的文字样式或者增加点击事件。

<LegacyCompatTable metadata="elements/nested-text" />

### 嵌套 image

[嵌套 image](/api/elements/built-in/text.html#嵌套-image)：`<text>` 内的子元件，用于图文混排。
