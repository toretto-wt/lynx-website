...
如果开发者需要自定义的上下文菜单，需要设置 `custom-context-menu` 属性，并且通过 `bindselectionchange` 事件和 `getTextBoundingRect` 方法来确定自定义上下文菜单的显示位置。
...
进一步的，如果你的需求是支持跨多个 `<text>` 元件进行选中并复制内容，需开发者自定义文本选择逻辑：
...
组件挂载时获取所有可选中的文本节点信息，包含节点的 id 和宽高位置等信息
...
设置是否开启自定义文本选择功能，开启时元件不再处理选择复制相关手势逻辑，需要开发者通过 `setTextSelection` 等 API 控制
