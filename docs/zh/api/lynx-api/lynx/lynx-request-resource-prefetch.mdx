...
- 该 API 用于前端主动发起 CDN 资源的预加载，目前只支持 _image_, _video_, _audio_, _font_ 四种资源类型。
...
- `code`：类型 `number`，状态码，它可能取值如下
  - `0`：成功
  - `32101`：参数错误
  - `32102`：Image prefetch helper 不存在
  - `32103`：Resource service 不存在
  - `32104`：等待预加载完成超时（仅当 `config.awaitComplete = true` 时可能返回）
...
控制是否需要等待预加载完成，以及等待的超时时间：

- `awaitComplete`（可选）：类型 `boolean`，默认 `false`。为 `true` 时，`callback` 会等待所有资源预加载完成后再触发；若超时则会返回 `32104`。
- `awaitTimeout`（可选）：类型 `number`，单位 ms，默认 `60000`。仅在 `awaitComplete = true` 时生效。
...
- 当出现参数错误时，`code` 会返回 `32101` 错误码，并且在 `msg` 中有详细错误信息。
- 当 Image prefetch helper 不存在时，`code` 会返回 `32102`。
- 当 Resource service 不存在时，`code` 会返回 `32103`。
- 当开启 `awaitComplete` 且等待超时时，`code` 会返回 `32104`。
